...
function Orderpage() {
  ...
  const handleEditSubmit = (event) => {
    ...
    if (Number(quantity) > availableQuantity) {
      toast.error(
        `Insufficient product quantity. Available: ${availableQuantity}, Requested: ${quantity}`
      );
      return;
    }
  ...
  const handleProductChange = (e) => {
    ...
    // Find the selected product and set available quantity
    const selectedProd = getallproduct?.find(
      (p) => p._id === selectedProductId
    );
    setAvailableQuantity(selectedProd?.quantity || 0);
  };
  ...
  const handleEditClick = (order) => {
    ...
    // Safely extract product ID from nested structure
    const productId =
      order.Product?.product?._id || order.Product?.product || "";
    setProduct(productId);

    // Find the product and set available quantity
              ...
              <tbody className="bg-base-100">
                {Array.isArray(displayOrder) && displayOrder.length > 0 ? (
                  displayOrder.map((order, index) => (
                    <tr key={order._id || index} className="bg-base-100">
                      <td className="px-3 py-2 border">{index + 1}</td>
                      <td className="px-3 py-2 border">
                        {order.Product?.product?.name || "N/A"}
                      </td>
                      <td className="px-3 py-2 border">
                        {order.Product?.quantity || "N/A"}
                      </td>
                      <td className="px-3 py-2 border">
                  ...
                  <tr>
                    <td colSpan="10" className="bg-base-100 text-center py-4">
                      No Order found.
                    </td>
                  </tr>
                )}
