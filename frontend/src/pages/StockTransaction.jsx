...
function StockTransaction() {
  ...
  const submitstocktranscation = async (event) => {
    ...
    if (!product || !type || !quantity || !supplier) {
      toast.error(
        "Please fill in all required fields: Product, Type, Quantity, and Supplier"
      );
      return;
    }
    ...
    dispatch(createStockTransaction(StocksData))
      ...
      .catch((error) => {
        toast.error(error || "Stock add unsuccessful");
      });
  };
                ...
                {Array.isArray(displaystock) && displaystock.length > 0 ? (
                  displaystock.map((Stocks, index) => (
                    <tr key={Stocks._id}>
                      ...
                      <td className="px-3 py-2 border">{Stocks.type}</td>
