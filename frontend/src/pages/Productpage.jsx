  ...
  const handleEditSubmit = (event) => {
    ...
    dispatch(EditProduct({ id: selectedProduct._id, updatedData }))
      ...
      .then(() => {
        ...
        resetForm();
        // Refresh the product list to show updated product
        dispatch(gettingallproducts());
      })
      .catch((error) => {
  ...
  const submitProduct = async (event) => {
    ...
    dispatch(Addproduct(productData))
      ...
      .then(() => {
        ...
        resetForm();
        setIsFormVisible(false);
        // Refresh the product list to show new product
        dispatch(gettingallproducts());
      })
      .catch((error) => {
    ...
    <div className="bg-base-100 min-h-screen">
      ...
      <div className="mt-10 flex">
          ...
          <p className="text-white font-bold pt-2 ml-24">
            ...
            {Array.isArray(getallproduct)
              ? getallproduct.reduce((totalAmount, product) => {
                  return totalAmount + (product.Price || 0);
                }, 0)
              : "0"}
          </p>
        </div>
        <div className="bg-blue-950 bg-base-100 w-56 rounded-xl ml-10 block h-24">
          ...
          <p className="text-white font-bold pt-2 ml-24">
            {" "}
            {Array.isArray(getallCategory) ? getallCategory.length : "0"}
          </p>
        </div>
      </div>
