...
const createOrder = async (req, res) => {
  try {
    ...
    if (productRecord.quantity < quantity) {
      console.log(
        `Insufficient stock: Available=${productRecord.quantity}, Requested=${quantity}`
      );
      return res.status(400).json({
        message: `Insufficient product quantity. Available: ${productRecord.quantity}, Requested: ${quantity}`,
...
const updatestatusOrder = async (req, res) => {
  try {
    ...
    // Validate structure
    if (updates.Product) {
      // Ensure Product is an object with product, quantity, and price
      if (
        ...
        return res.status(400).json({
          message: "Product object must contain product, quantity, and price",
        });
      }
      // Calculate new total amount
      updates.totalAmount = updates.Product.quantity * updates.Product.price;
    }
